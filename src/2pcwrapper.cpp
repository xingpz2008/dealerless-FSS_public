...
void cross_term_gen(int party_id, GroupElement* input, GroupElement* output, bool hold_arithmetic, int size, Peer* player){
    ...
    if (hold_arithmetic){
        GroupElement* cot_output = new GroupElement[iteration_size];
        for (int i = 0; i < iteration_size; i++){
            cot_output[i].bitsize = input[0].bitsize;
        }
        uint64_t constant;
        GroupElement* data_to_send = new GroupElement[iteration_size];
        for (int i = 0; i < size; i++){
...
void B2A(int party_id, u8* x, GroupElement* y, int size, int bw_y, Peer* player){
    ...
    if ((party_id - 2) == 0) {
        GroupElement *corr_data = new GroupElement[size];
        for (int i = 0; i < size; i++) {
            corr_data[i].bitsize = bw_y;
            corr_data[i].value = (-2 * uint64_t(x[i])) & mask;
            mod(corr_data[i]);
        }
        player->send_cot(corr_data, y, size, false);
