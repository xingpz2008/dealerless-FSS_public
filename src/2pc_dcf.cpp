...
void evaliDCF(int party, GroupElement *res, GroupElement idx, const iDCFKeyPack& key, bool masked){
    ...
    std::vector<GroupElement> Wcw_list(idx.bitsize - 1);
    std::vector<u8> tau_list(2 * (idx.bitsize - 1));
    std::vector<block> CW_list(idx.bitsize - 1);
...
void evaliDCF(int party, GroupElement *res, GroupElement idx, const iDCFKeyPack& key){
    ...
    const GroupElement* a = key.a;
    const GroupElement* b = key.b;
    const GroupElement* c = key.c;

    // Create 2 multiplication list
    std::vector<GroupElement> multi_list_0(Bin);
    std::vector<GroupElement> multi_list_1(Bin);
    ...
    std::vector<GroupElement> multi_list_output(Bin);
    beaver_mult_online(party, multi_list_0.data(), multi_list_1.data(), a, b, c,
                       multi_list_output.data(), Bin, peer);
...
void evaliDCF(int party, GroupElement* res, const GroupElement* idx, const iDCFKeyPack* keyList, int size, int max_bitsize){
    ...
    std::vector<GroupElement> beta_0(size);
    std::vector<GroupElement> mask(size);
    std::vector<GroupElement> masked_idx(size);
    std::vector<GroupElement> Wcw_list(size * (max_bitsize - 1));
    std::vector<u8> tau_list(size * 2 * (max_bitsize - 1));
    std::vector<block> CW_list(size * (max_bitsize - 1));
    std::vector<GroupElement> a(size * max_bitsize);
    std::vector<GroupElement> b(size * max_bitsize);
    std::vector<GroupElement> c(size * max_bitsize);
    std::vector<GroupElement> multi_list_0(max_bitsize * size);
    std::vector<GroupElement> multi_list_1(max_bitsize * size);
    std::vector<u8> t(size);
    std::vector<GroupElement> layerRes(size);
    std::vector<block> level_st(size);
    std::vector<u8> level_t(size);
    std::vector<GroupElement> multi_list_output(size * max_bitsize);
    ...
    reconstruct(size, masked_idx.data(), max_bitsize);

    // Execute batched DCF evaluation. This loop is intentionally serial: the
    // OpenMP variant regressed in localhost A/B timing because the per-item
    // work is small before the batched Beaver multiplication.
    for (int i = 0; i < size; i++){
        for (int j = 0; j < idx[i].bitsize - 1; j++){
            ...
            st[i] = level_st[i];
            t[i] = level_t[i];
            multi_list_0[1 + j + i * max_bitsize].value = (uint64_t)party - idx[i][j + 1];
            multi_list_0[1 + j + i * max_bitsize].bitsize = idx[i].bitsize;
            multi_list_1[1 + j + i * max_bitsize] = layerRes[i];
        }
    }

    beaver_mult_online(party, multi_list_0.data(), multi_list_1.data(), a.data(),
                       b.data(), c.data(), multi_list_output.data(),
                       size * max_bitsize, peer);
