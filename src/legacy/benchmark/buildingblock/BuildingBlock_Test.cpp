...
int main(int argc, char **argv){
    ...
    amap.arg("m", CTN_SIZE, "Containment knot count");
    amap.parse(argc, argv);

    if (CTN_SIZE <= 0) {
        throw std::invalid_argument("m/CTN_SIZE must be positive");
    }
    std::vector<GroupElement> ctn_knots;
    ctn_knots.reserve(CTN_SIZE);
    for (int i = 0; i < CTN_SIZE; i++) {
        ctn_knots.emplace_back(0.5 * static_cast<double>(i + 1), Bin, scale);
    }
    ...
    switch (function_choice) {
        case MODULAR:{
            key.key1 = dfss::legacy::modularOffline(
                party, GroupElement(MODULAR_N, Bin), Bout);
            break;
        }
        case TRUNCATE_AND_REDUCE:{
            key.key2 = dfss::legacy::truncateOffline(party, Bin, TR_S);
            break;
        }
        case CONTAINMENT:{
            key.key3 = dfss::legacy::containmentOffline(
                party, Bout, ctn_knots.data(), CTN_SIZE);
            break;
        }
        case DIGDEC:{
            key.key4 = dfss::legacy::digdecOffline(
                party, Bin, DIGDEC_NEW_BITSIZE);
            break;
        }
        case PUBLUT:{
            key.key5 = dfss::legacy::publicLutOffline(party, Bin, Bout);
            break;
        }
        case PRILUT:{
            ...
            key.key6 = dfss::legacy::privateLutOffline(
                party, Bin, Bout, table);
            delete[] table;
            break;
        }
        case APPROX:{
            ...
            key.key7 = dfss::legacy::splinePolyApproxOffline(party, Bin, Bout, coefList,
    ...
    switch (function_choice) {
        case MODULAR:{
            dfss::legacy::modular(party, x, MODULAR_N, key.key1);
            break;
        }
        case TRUNCATE_AND_REDUCE:{
            dfss::legacy::truncate(party, x, TR_S, key.key2);
            break;
        }
        case CONTAINMENT:{
            ...
            dfss::legacy::containment(party, x, containment_output, CTN_SIZE, key.key3);
        ...
        case DIGDEC:{
            ...
            dfss::legacy::digdec(
                party, x, digdec_output, DIGDEC_NEW_BITSIZE, key.key4);
        ...
        case PUBLUT:{
            ...
            dfss::legacy::publicLut(party, x, table, shifted, (1 << Bin), Bout, key.key5);
            delete[] table;
            delete[] shifted;
            break;
        }
        case PRILUT:{
            dfss::legacy::privateLut(party, x, key.key6);
            break;
        }
        case APPROX:{
            dfss::legacy::splinePolyApprox(party, x, key.key7);
            break;
        }
    }
