...
SineKeyPack sine_offline(int party_id, int Bin, int Bout, int scale, bool using_lut,
    ...
    output.ModExtendKey = dfss::legacy::legacyRingExtendOffline(party_id, scale + 1, scale + 2);
    // First, we need a mod key
    output.ModKey = dfss::legacy::modularOffline(party_id, GroupElement(2, Bin, scale), 2 + scale);
    int MTList_len = 2;
    if (using_lut){
        ...
        output.approx_deg = -1;
        // create dig dec keys
        output.DigDecKey = dfss::legacy::digdecOffline(party_id, scale - 1, digdec_new_bitsize);
        ...
        for (int i = 0; i < digdec_segNum; i++){
            ...
            output.EvalAllKeyList[i] =
                dfss::legacy::publicLutOffline(party_id, digdec_new_bitsize, product_bits);
        }
        output.LUTProductTRKeyList = makeKeyArray<LegacyTRKeyPack>(2);
        for (int i = 0; i < 2; i++) {
            output.LUTProductTRKeyList[i] =
                dfss::legacy::truncateOffline(party_id, product_bits, scale);
        }
        // Prepare multiplication triples for digit-decomposition combination.
        ...
        // This segment size is for truncation in the approximation path.
        ...
        output.SplineApproxKey = dfss::legacy::splinePolyApproxOffline(
            party_id, scale - 1, approx_eval_bits, publicCoefficientList.data(),
            approx_deg, approx_segNum, scale);
    }
    // One dfss::containment key determines the half-period interval. Its output is
    ...
    output.CtnKey =
        dfss::legacy::containmentOfflinePublic(party_id, ctn_bits, first_knots_list.data(), 3);
    ...
    // Digit-decomposition combination triples use the evaluation ring.
...
GroupElement sine(int party_id, GroupElement input, const SineKeyPack& key){
    ...
    GroupElement x_mod = dfss::legacy::legacyRingExtend(
        party_id, segment(input, key.scale + 1).second, key.scale + 2,
        key.ModExtendKey);
    std::array<GroupElement, 4> v;
    dfss::legacy::containment(party_id, x_mod, v.data(), 3, key.CtnKey);
    ...
    if (key.using_lut){
        // Decompose the fractional part before the sub-LUT evaluations.
        ...
        dfss::legacy::digdec(party_id, x_frac, x_seg.data(), key.digdec_new_bitsize, key.DigDecKey);
        ...
        for (int i = 0; i < digdec_segNum; i++){
            ...
            sin_lut_output[i] = dfss::legacy::publicLut(party_id, x_seg[i], publicSinList[i],
        ...
        y_0 = dfss::legacy::splinePolyApprox(party_id, x_frac, key.SplineApproxKey);
    }
...
TangentKeyPack tangent_offline(int party_id, int Bin, int Bout, int scale, bool using_lut,
    ...
    output.Bout = Bout;
    output.LUTProductTRKeyList = NULL;
    output.ModExtendKey = dfss::legacy::legacyRingExtendOffline(party_id, scale, scale + 1);
    // First, we need a mod key
    output.ModKey = dfss::legacy::modularOffline(party_id, GroupElement(1, Bin, scale), 1 + scale);
    int MTList_len = 2;
    if (using_lut){
        ...
        for (int i = 0; i < 1; i++){
            ...
            // Here we need random idx at r, which do not require mask because it was used in EvalAll.
            output.EvalAllKeyList[i] = dfss::legacy::publicLutOffline(party_id, (scale - 1), Bout);
        }
    }else{
        const int approx_eval_bits =
            fixed_point_approx_eval_bits(Bout, scale);
    ...
    output.CtnKey =
        dfss::legacy::containmentOfflinePublic(party_id, ctn_bits, first_knots_list.data(), 1);
...
GroupElement tangent(int party_id, GroupElement input, const TangentKeyPack& key){
    ...
    dfss::legacy::containment(party_id, x_mod, v.data(), 1, key.CtnKey);
    ...
    GroupElement y_0 = GroupElement(0, input.bitsize);
    if (key.using_lut){
        // Tangent LUT uses the whole fractional part as a single segment.
        int digdec_segNum = 1;
        ...
        for (int i = 0; i < digdec_segNum; i++){
            tan_lut_output[i] = dfss::legacy::publicLut(party_id, x_seg[i], publicTanList[i],
                                        shifted_vector_list[i], lut_size,
...
ProximityKeyPack proximity_offline(int party_id, int Bin, int scale, bool using_lut, int digdec_new_bitsize,
    ...
    output.ProductTRKeyList = makeKeyArray<LegacyTRKeyPack>(4);
    output.ProductExtendKeyList = makeKeyArray<LegacyComparisonKeyPack>(6);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        output.ProductTRKeyList[i] = dfss::legacy::truncateOffline(party_id, product_bits, scale);
    }
    for (int i = 0; i < 4; i++) {
        output.ProductExtendKeyList[i] =
            dfss::legacy::legacyRingExtendOffline(party_id, Bin, product_bits);
    }
    for (int i = 4; i < 6; i++) {
        output.ProductExtendKeyList[i] =
            dfss::legacy::legacyRingExtendOffline(party_id, Bin, product_bits);
...
GroupElement proximity(int party_id, GroupElement xA, GroupElement yA, GroupElement xB, GroupElement yB,
    ...
    GroupElement front_extended = dfss::legacy::legacyRingExtend(
        party_id, _front_output, product_bits, key.ProductExtendKeyList[0]);
    GroupElement back_cos_0_extended = dfss::legacy::legacyRingExtend(
        party_id, _back_output_0, product_bits, key.ProductExtendKeyList[1]);
    GroupElement back_sin_extended = dfss::legacy::legacyRingExtend(
        party_id, _back_output_2, product_bits, key.ProductExtendKeyList[2]);
    GroupElement back_cos_1_extended = dfss::legacy::legacyRingExtend(
    ...
    GroupElement front_output = dfss::legacy::truncate(party_id, batch_mul_output[0], scale,
                                                    key.ProductTRKeyList[0]);
    GroupElement back_output_0 = dfss::legacy::truncate(party_id, batch_mul_output[1], scale,
                                                     key.ProductTRKeyList[1]);
    GroupElement back_output_1 = dfss::legacy::truncate(party_id, batch_mul_output[2], scale,
                                                     key.ProductTRKeyList[2]);
    GroupElement back_output = beaver_mult_online(
        party_id,
        dfss::legacy::legacyRingExtend(party_id, back_output_0, product_bits,
                    key.ProductExtendKeyList[4]),
        dfss::legacy::legacyRingExtend(party_id, back_output_1, product_bits,
                    key.ProductExtendKeyList[5]),
        key.Alist[3], key.Blist[3], key.Clist[3], peer);
    GroupElement back_output_truncated = dfss::legacy::truncate(party_id, back_output, scale,
