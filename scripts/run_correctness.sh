...
Options:
  ...
  --skip-build         use the existing CORRECTNESS_TEST binary
  --quiet              print only the final summary
  -h, --help           show this help

Case numbers:
  0 all, 1 DPF, 2 DCF, 3 comparison, 4 power-of-two modular,
  5 truncate, 6 containment, 7 public LUT,
  8 private LUT, 9 spline, 10 digit decomposition,
  11 constrained comparison, 12 trigonometric, 13 proximity
USAGE
}
...
LOG_DIR="${BUILD_DIR}/correctness-logs"
...
if [[ "${BUILD}" -eq 1 ]]; then
    BUILD_ARGS=(--build "${BUILD_DIR}" --target CORRECTNESS_TEST)
    if [[ -n "${JOBS}" ]]; then
...
TEST_BIN="${BUILD_DIR}/2pc_test/CORRECTNESS_TEST/CORRECTNESS_TEST"
if [[ ! -x "${TEST_BIN}" ]]; then
...
echo "Running correctness case ${TEST_CASE} on port ${PORT}"
